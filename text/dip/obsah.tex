...
\begin{figure}[htb]
    \centering
    \subfigure[Šum typu sůl a pepř a náhodný impulzní šum]{
        \hskip1cm
        \centering
...
\begin{figure}[tbp]
    ...
    \caption{Vliv způsobu inicializace prediktorů fitness pro různé intenzity šumu typu sůl a pepř}
...
\begin{figure}[p]
    ...
    \caption{Vliv koeficientů změny velikosti prediktorů v~případě, že je velikost změny určena podle celkového počtu případů fitness}
    \label{plotAbsCoef}
\end{figure}
...
\begin{table}[hp]
    \caption{Srovnání rychlosti $\mathit{CGP_{STD}}$, $\mathit{FP_{REL}}$ a $\mathit{FP_{ABS}}$. Uvedené hodnoty jsou průměr procesorového času (součtu všech vláken) v~minutách ze 100 nezávislých běhů}
    \label{tabUtime}
    \renewcommand{\arraystretch}{1.2}
    \centering
    \small
...
\chapter{Závěr}
\label{chConclusions}
\enlargethispage{4\baselineskip} % strange hack to get rid of blank page before bibliography
