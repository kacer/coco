...
\begin{figure}[htb]
    \centering\includegraphics[width=0.85\textwidth]{fig/ea.pdf}
...
\begin{figure}[htb]
    \centering\includegraphics[width=0.6\textwidth]{fig/cgp.pdf} \\
...
\begin{figure}[htb]
    \centering\includegraphics[width=0.66\textwidth]{fig/filter2b.pdf}
...
\begin{figure}[htb]
    ...
    \subfigure[Vstupní obrázek]{
        \hskip0.75cm
        \includegraphics[width=0.21\textwidth]{fig/testimages/cameraman.png}
        \hskip0.75cm
    }
...
\begin{figure}[htb]
    ...
    \centering\includegraphics[width=0.9\textwidth]{fig/coev3full.pdf}
    \caption{Schéma koevoluce CGP a prediktorů fitness~\cite{SikuEuroGP}.}
    \label{obrKoevoluce}
\end{figure}

Evoluce kandidátních programů probíhá pomocí kartézského genetického programování. V~případě koevoluce se pro ohodnocení kandidátních řešení používají dvě různé fitness funkce: $f_{\mathit{exact}}$, ve které se používá celá trénovací množina, a $f_{\mathit{predicted}}$, omezená pouze na některé případy fitness.
...
\begin{figure}[htb]
    \centering
    \subfigure[Zdrojový (zašuměný) a cílový (originální) obrázek]{
        \hskip1cm
        \centering\includegraphics[width=0.85\textwidth]{fig/simdlayout-window}
        \hskip1cm
    }
    \vskip1\baselineskip
    \subfigure[Struktura dat pro sekvenční zpracování]{
        \centering
        \includegraphics[width=0.85\textwidth]{fig/simdlayout-seq}
    }
    \vskip1\baselineskip
    \subfigure[Struktura dat pro vektorové zpracování]{
        \centering
        \includegraphics[width=0.85\textwidth]{fig/simdlayout-vect}
    }
...
%V této kapitole jsou nejprve v části \ref{secExpProblems} uvedeny zvolené testovací problémy a v části \ref{secExpEvoParams} použité hodnoty parametrů kartézského genetického programování a genetického programování pro prediktory fitness, které jsou použity ve všech experimentech. Následující část \ref{secExpColearnParams} se zabývá volbou hodnot parametrů souběžného učení a jejich vlivem na kvalitu výsledných filtrů a rychlost evoluce. V dalších částech je popsána schopnost adaptace velikosti prediktoru na řešenou úlohu (část \ref{secExpAdapt}) a četnost jednotlivých devítiokolí v prediktorech (\ref{secExpPredhist}). Srovnání navrženého algoritmu se standardním (koevolučním) CGP je v části \ref{secExpComparison} a závěrečné části \ref{secExpFilters} jsou uvedeny ukázky nejlepším filtrů nalezených pomocí souběžného učení v koevolučním CGP.
...
\begin{figure}[htb]
    ...
    \subfigure[Šum sůl a pepř a výstřelový šum]{
        \hskip1cm
        \centering
            \includegraphics[width=0.3\textwidth]{fig/testimages/lena_gray.png}
    ...
    \subfigure[Detekce hran]{
        \hskip1cm
        \centering
            \includegraphics[width=0.3\textwidth]{fig/testimages/cameraman.png}
...
\begin{figure}[htb]
    \centering
    \subfigure[Kvalita nalezených filtrů a doba běhu programu (součet všech vláken)]{
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-zerocoef-30kg-psnr.pdf}
        \hskip0.5cm
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-zerocoef-30kg-utime.pdf}
    }
    \subfigure[Průběh velikosti prediktorů (průměr ze 100 běhů)]{
        \captionsetup{aboveskip=0pt}
        \centering
        \includegraphics[width=0.85\textwidth]{fig/plot/tuning-zerocoef-30kg-predlen.pdf}
    }
...
\begin{figure}[htb]
    \centering
    \subfigure{
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-decrcoef-30kg-psnr.pdf}
        \hskip0.5cm
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-decrcoef-30kg-utime.pdf}
    }
...
\begin{figure}[htb]
    \centering
    \subfigure{
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-slowcoef-30kg-psnr.pdf}
        \hskip0.5cm
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-slowcoef-30kg-utime.pdf}
    }
    \caption{Vliv parametru $c_{+1}$ (pravidlo $0 < v~\leq v_\mathit{slow}: \mathit{UsedGenes} \leftarrow \mathit{UsedGenes} \cdot c_{+1}$)}
    \label{plotSlowcoef-30kg}
\end{figure}

\begin{figure}[htb]
    \centering
    \subfigure[Při počáteční velikosti prediktoru 50\,\%]{
        \centering
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-fastcoef-i50-30kg-psnr.pdf}
        \hskip0.5cm
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-fastcoef-i50-30kg-utime.pdf}
    }
    \subfigure[Při počáteční velikosti prediktoru 3\,\%]{
        \centering
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-fastcoef-i3-30kg-psnr.pdf}
        \hskip0.5cm
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-fastcoef-i3-30kg-utime.pdf}
        \label{plotFastcoef-30kg-i3}
    }
    \caption{Vliv parametru $c_{+2}$ (pravidlo $v > v_\mathit{slow}: \mathit{UsedGenes} \leftarrow \mathit{UsedGenes} \cdot c_{+2}$)}
...
\begin{figure}[htb]
    ...
    \captionsetup{aboveskip=0pt}
    \subfigure{
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-interval-30kg-psnr.pdf}
        \hskip0.5cm
        \includegraphics[width=0.475\textwidth]{fig/plot/tuning-interval-30kg-utime.pdf}
    }
...
Dosažené výsledky po 30 tisících generacích jsou na grafech~\ref{plotBwalg-30kg}. Ukazuje se, že u~všech zkoumaných variant je kvalita filtrů srovnatelná. Také potřebný čas běhu je podobný, s~výjimkou varianty \emph{mean7w}, u~které je evoluce v~průměru 2,2krát pomalejší než u~\emph{last}.

\begin{figure}[htb]
    \centering
    \captionsetup{aboveskip=0pt}
    \subfigure{
        \includegraphics[width=0.475\textwidth]{fig/plot/bwalg-30kg-psnr.pdf}
        \hskip0.5cm
        \includegraphics[width=0.475\textwidth]{fig/plot/bwalg-30kg-utime.pdf}
    }
...
\begin{figure}[tbp]
    \centering
    \subfigure{
        \includegraphics[width=\textwidth]{fig/plot/rndperm-100kg-psnr.pdf}
    }
    \vskip\baselineskip
    \subfigure{
        \includegraphics[width=\textwidth]{fig/plot/rndperm-100kg-utime.pdf}
    }
    \caption{Vliv způsobu inicializace prediktorů fitness. Výsledky po 100~000 generacích u~šumu typu sůl a pepř různé intenzity.}
    \label{plotRndPerm-100kg}
\end{figure}

\subsection{Změna velikosti prediktoru podle celkového počtu případů fitness}

Experimentovalo se také s~druhou sadou pravidel, uvedených v~tabulce~\ref{tabBaldwinRulesAbs}, kdy je změna velikosti prediktoru vypočtena z~celkového počtu případů fitness. Byly testovány hodnoty od $-0.10$ po $+0.10$ s~krokem po $0.01$ pro koeficienty $c_0$, $c_{-1}$, $c_{+1}$ a $c_{+2}$. Ve všech případech platí, že různé hodnoty nemají vliv na kvalitu filtrů, ale jen na rychlost programu. Obecně platí, že při každé změně se prediktor zvětší či zmenší o~větší počet případů fitness než v~případě dosud popisované relativní změny a lze zde proto pozorovat zvýšení potřebného času u~všech koeficientů. Potřebný čas stoupá až od určité hodnoty koeficientu, konkrétně:
...
%Obecně se dá říci, že tato sada pravidel je citlivější na vyšší hodnoty koeficientů.
...
\begin{figure}[p]
    \captionsetup{aboveskip=0pt}
    \centering
    \subfigure[Vliv parametru $c_{0}$ (pravidlo $\left|v\right| \leq v_\mathit{zero}: \mathit{UsedGenes} \leftarrow \mathit{UsedGenes} + N \cdot c_0$)]{
        \centering
    ...
    \subfigure[Vliv parametru $c_{-1}$ (pravidlo $v < 0: \mathit{UsedGenes} \leftarrow \mathit{UsedGenes} + N \cdot c_{-1}$)]{
        \centering
    ...
    \subfigure[Vliv parametru $c_{+1}$ (pravidlo $0 < v~\leq v_\mathit{slow}: \mathit{UsedGenes} \leftarrow \mathit{UsedGenes} + N \cdot c_{+1}$)]{
        \centering
    ...
    \subfigure[Vliv parametru $c_{+2}$ (pravidlo $v > v_\mathit{slow}: \mathit{UsedGenes} \leftarrow \mathit{UsedGenes} + N \cdot c_{+2}$)]{
        \centering
    ...
    \caption{Vliv koeficientů změny velikosti prediktorů podle celkového počtu případů fitness.}
    \label{plotAbsCoef}
\end{figure}


\section{Adaptace velikosti prediktoru na úlohu}
\label{secExpAdapt}
...
\begin{figure}[htb]
    ...
    \subfigure[5\% šum typu sůl a pepř]{
        \centering
            \includegraphics[width=0.475\textwidth]{fig/plot/initial-size/initial-size-small-5-100kg.pdf}
    }
    \hfill
    \subfigure[15\% šum typu sůl a pepř]{
        \centering
            \includegraphics[width=0.475\textwidth]{fig/plot/initial-size/initial-size-small-15-100kg.pdf}
    }
    \subfigure[50\% šum typu sůl a pepř]{
        \centering
            \includegraphics[width=0.475\textwidth]{fig/plot/initial-size/initial-size-small-50-100kg.pdf}
    }
    \hfill
    \subfigure[Detektor hran]{
        \centering
            \includegraphics[width=0.475\textwidth]{fig/plot/initial-size/initial-size-small-sobel-100kg.pdf}
    }
    \caption{Vývoj velikosti prediktorů fitness při různé počáteční velikosti.}
    \label{plotInitialSize}
\end{figure}

\subsection{Vztah fitness a velikosti prediktoru}
...
Toto pořadí platí v~celém průběhu evoluce. Pokud se použije počáteční velikost prediktorů 50\,\%, celkový počet úprav je nižší, v~průměru jich bylo 600. Pořadí pravidel je stejné, s~výjimkou prvních 10 tisíc generací, kdy se nejvíce používají koeficienty $c_{+2}$ a $c_{+1}$.

%Kolikrát se uplatnil každý koeficient v různých fázích evoluce je zobrazeno na grafu na obrázku~\ref{plotRuleCount}.

%\begin{figure}[htb]
%    \centering
%    \includegraphics[width=0.475\textwidth]{fig/plot/rulecount-small-i3.pdf}
%    \caption{Počet použití jednotlivých koeficientů změny velikosti prediktoru v průběhu evoluce. Průměrné hodnoty za 100 běhů.}
%    \label{plotRuleCount}
%\end{figure}
...
K~testování byly použity tři obrázky poškozené šumem typu sůl a pepř, dva byly poškozené po celé ploše šumem s~intenzitou 15 a 50 procent, třetí obrázek byl částečně poškozen 25\% šumem. Pro úlohu detekce hran byl zvolen obrázek \uv{kameraman}. Pro každý obrázek bylo spuštěno 100 nezávislých běhů.
...
\begin{figure}[hbt]
    ...
    \newcommand\margin{}%\hskip0.3cm}
    \newcommand\gap{\hskip0.2cm}
    \newcommand\gapCol{\hfill}
    \newcommand\gapRow{}%\\\vskip\baselineskip}
    \subfigure[15\% šum typu sůl a pepř]{
        % 137030, 358005
        \margin
        \includegraphics[width=\size]{fig/predhist/sp15.png}
        \gap
        \includegraphics[width=\size]{fig/predhist/15.png}
        \margin
    }
    \gapCol
    \subfigure[Částečné poškození 25\% šumem typu sůl a pepř]{
        \margin
        \includegraphics[width=\size]{fig/predhist/sp25part.png}
        \gap
        \includegraphics[width=\size]{fig/predhist/25part.png}
        \margin
    }
    \gapRow
    \subfigure[50\% šum typu sůl a pepř]{
        % 123 739, 2 686
        \margin
        \includegraphics[width=\size]{fig/predhist/sp50.png}
        \gap
        \includegraphics[width=\size]{fig/predhist/50.png}
        \margin
    }
    \gapCol
    \subfigure[Detektor hran]{
...
\begin{table}[h]
    ...
    \renewcommand{\arraystretch}{1.2}
    \small\centering
    \subtable[$\mathit{FP_{REL}}$: změna podle současné velikosti prediktoru]{
        \label{tabParamsRel}
        ...
        \begin{minipage}[t]{.7\textwidth}
            \centering\begin{tabular}{ll}
                ...
                četnosti změny velikosti & jen při zvýšení $f_\mathit{predicted}$ \\
    ...
    \vskip\baselineskip
    \subtable[$\mathit{FP_{ABS}}$: změna podle celkového počtu případů fitness]{
        \label{tabParamsAbs}
        \begin{minipage}[t]{.28\textwidth}
            \centering\begin{tabular}{ll}
                \toprule
...
\begin{figure}[h!]
    \centering
    %\newcommand\size{0.23\textwidth}
    %\newcommand\gap{0.02667\textwidth}
    ...
    \includegraphics[width=\size]{fig/testimages/lena_gray.png}\hfill
    \includegraphics[width=\size]{fig/testimages/livingroom.png}
    \\\vskip\gap
...
\begin{table}[hp]
    \caption{Srovnání rychlosti $\mathit{CGP_{STD}}$, $\mathit{FP_{REL}}$ a $\mathit{FP_{ABS}}$. Uvedené hodnoty jsou průměr procesorového času (součtu všech vláken) v~minutách pro 100 běhů.}
    \label{tabUtime}
    \renewcommand{\arraystretch}{1.2}
    \centering
    \small
    \subtable[Šum typu sůl a pepř]{
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}} l*{8}{r}}
            \toprule
            Intenzita & 5\,\% & 10\,\% & 15\,\% & 20\,\% & 25\,\% & 30\,\% & 35\,\% & 40\,\% \\
            \midrule
            $\mathit{CGP_{STD}}$ & 65,15 & 64,98 & 64,91 & 65,22 & 65,11 & 64,83 & 64,76 & 64,54 \\
            \midrule
            $\mathit{FP_{REL}}$ & 26,89 & 18,54 & 11,85 & 10,59 & 7,29 & 6,87 & 6,84 & 6,73  \\
            Zrychlení & 2,42$\times$ & 3,50$\times$ & 5,48$\times$ & 6,16$\times$ & 8,94$\times$ & 9,44$\times$ & 9,46$\times$ & 9,59$\times$  \\
            \midrule
            $\mathit{FP_{ABS}}$ & 23,05 & 18,11 & 13,64 & 10,72 & 9,58 & 9,84 & 9,80 & 9,77  \\
            Zrychlení & 2,83$\times$ & 3,59$\times$ & 4,76$\times$ & 6,09$\times$ & 6,80$\times$ & 6,59$\times$ & 6,61$\times$ & 6,61$\times$  \\
            \bottomrule
        \end{tabular*}
    }
    \subtable{
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}} l*{8}{r}}
            ...
            \midrule
            $\mathit{CGP_{STD}}$ & 64,85 & 64,98 & 64,65 & 64,53 & 64,80 & 64,73 & 64,68 & 64,90  \\
            \midrule
            $\mathit{FP_{REL}}$ & 7,01 & 6,39 & 5,53 & 5,10 & 4,56 & 4,39 & 4,35 & 4,51  \\
            Zrychlení & 9,26$\times$ & 10,17$\times$ & 11,69$\times$ & 12,64$\times$ & 14,20$\times$ & 14,75$\times$ & 14,86$\times$ & 14,40$\times$ \\
            \midrule
            $\mathit{FP_{ABS}}$ & 9,51 & 9,49 & 9,94 & 9,48 & 9,78 & 9,56 & 9,24 & 8,96  \\
            Zrychlení & 6,82$\times$ & 6,85$\times$ & 6,50$\times$ & 6,80$\times$ & 6,62$\times$ & 6,77$\times$ & 7,00$\times$ & 7,24$\times$  \\
            \bottomrule
        \end{tabular*}
    }
    \vskip\baselineskip
    \addtocounter{subtable}{-1}
    \subtable[Výstřelový šum]{
        \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}} l*{8}{r}}
            \toprule
            Intenzita & 5\,\% & 10\,\% & 15\,\% & 20\,\% & 25\,\% & 30\,\% & 35\,\% & 40\,\% \\
            \midrule
...
\section{Nalezené filtry}
\label{secExpFilters}

Výstup a zapojení nejlepšího evolvovaného filtru pro 25\% šum typu sůl a pepř je na obrázku~\ref{obrFilterSP}. Pro srovnání je zobrazen i výstup mediánového filtru s~oknem $3\times3$. Oproti mediánovému filtru si evolvovaný filtr lépe poradil se shluky poškozených pixelů a zachovalo se více detailů. Také hodnota PSNR je vyšší, pro evolvovaný filtr činí 29,46 dB, kdežto u~mediánového filtru to je pouze 24,61 dB. Ve filtru je použita pouze část možných funkcí bloků, nejčastěji se vyskytuje sčítání vstupů. Ve několika případech jde o~součet primárního vstupu a konstanty 255, což v~případě 8bitových číslech bez znaménka odpovídá odečtení jedničky. Tato operace se na různých místech filtru provádí pro každý použitý primární vstup. Sníží se tak dynamický rozsah šumu -- v~případě černého pixelu (s~hodnotou 0) je výsledkem hodnota 255, v~případě bílého (255) je výsledkem 254. Podobný efekt má funkce $255 - i_1$, která je součástí množiny použitelných funkcí a ve filtru je také dvakrát použita.% (na obrázku je označená jako INV).

\begin{figure}[hbt]
    \centering
    \subfigure[Poškozený obrázek]{
        \includegraphics[width=0.25\textwidth]{fig/filters/sp25/noisy}
    }
    \hfill
    \subfigure[Mediánový filtr]{
        \includegraphics[width=0.25\textwidth]{fig/filters/sp25/median}
    }
    \hfill
    \subfigure[Evolvovaný filtr]{
        \includegraphics[width=0.25\textwidth]{fig/filters/sp25/evo}
    }
    \subfigure[Zapojení evolvovaného filtru]{
        \includegraphics[width=0.9\textwidth]{fig/filters/sp25/filter}
    }
...
\begin{figure}[htb]
    \centering
    \subfigure[Poškozený obrázek]{
        \includegraphics[width=0.25\textwidth]{fig/filters/impulse10/noisy}
    }
    \hfill
    \subfigure[Mediánový filtr]{
        \includegraphics[width=0.25\textwidth]{fig/filters/impulse10/median}
    }
    \hfill
    \subfigure[Evolvovaný filtr]{
        \includegraphics[width=0.25\textwidth]{fig/filters/impulse10/evo}
    }
    \subfigure[Zapojení evolvovaného filtru]{
        \includegraphics[width=0.9\textwidth]{fig/filters/impulse10/filter}
    }
...
\begin{figure}[htb]
    \centering
    \subfigure[Vstupní obrázek]{
        \includegraphics[width=0.25\textwidth]{fig/filters/sobel/source}
    }
    \hfill
    \subfigure[Sobelův detektor]{
        \includegraphics[width=0.25\textwidth]{fig/filters/sobel/sobel}
    }
    \hfill
    \subfigure[Evolvovaný detektor]{
        \includegraphics[width=0.25\textwidth]{fig/filters/sobel/evo}
    }
    \subfigure[Zapojení evolvovaného detektoru]{
        \includegraphics[width=0.9\textwidth]{fig/filters/sobel/filter}
    }
...
% Schéma a výstup nejlepšího nalezeného filtru pro výstřelový šum o stejné intenzitě 25\,\% je na obrázku~\ref{obrFilterImp}. U~tohoto šumu mají poškozené pixely náhodnou hodnotu. Je proto těžší jej odstranit, protože je těžší určit, zda je zpracovávaný pixel poškozený nebo ne. V tomto případě je PSNR evolvovaného filtru 25,75 dB, což je méně než u mediánového filtru, u kterého to je 26,66 dB. Oproti filtru pro sůl a pepř je zcela jiná skladba použitých funkcí. Vyskytují se pouze funkce minimum a maximum. Také jsou zde použity všechny primární vstupy, tj. všechny body z devítiokolí zpracovávaného pixelu.
% \begin{figure}[htb]
%     \centering
%     \subfigure[Poškozený obrázek]{
%         \includegraphics[width=0.3\textwidth]{fig/filters/impulse25/noisy}
%     }
%     \hfill
%     \subfigure[Mediánový filtr]{
%         \includegraphics[width=0.3\textwidth]{fig/filters/impulse25/median}
%     }
%     \hfill
%     \subfigure[Evolvovaný filtr]{
%         \includegraphics[width=0.3\textwidth]{fig/filters/impulse25/evo}
%     }
%     \subfigure[Zapojení evolvovaného filtru]{
%         \includegraphics[width=0.9\textwidth]{fig/filters/impulse25/filter}
%     }
%     \caption{Srovnání mediánového a evolvovaného filtru pro 25\% výstřelový šum.}
%     \label{obrFilterImp}
% \end{figure}
