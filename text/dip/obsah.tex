...
Přeložený program obsahuje všechny tři zmíněné implementace výpočtu CGP. Která implementace se použije při výpočtu, závisí na tom, jaké instrukční sady podporuje počítač a operační systém, na kterém je program spuštěn. To se zjišťuje pomocí instrukce \texttt{CPUID}. Přednost má implementace s~instrukcemi AVX2, kdy je naměřené zrychlení oproti sériové implementaci u~standardního CGP přibližně šestnáctinásobné. Další v~pořadí je implementace pomocí SSE2, u~níž je zrychlení zhruba desetinásobné. Pokud ani tuto instrukční sadu procesor nebo operační systém nepodporuje, použije se základní sekvenční implementace.
...
\begin{figure}[htb]
    ...
    \subfigure[Kvalita nalezených filtrů a doba běhu programu (součet všech vláken)]{
        \centering
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-zerocoef-30kg-psnr.pdf}
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-zerocoef-30kg-utime.pdf}
    }
    \subfigure[Průběh velikosti prediktorů (průměr ze 100 běhů)]{
        \centering
        \includegraphics[width=0.8\textwidth]{fig/plot/tuning-zerocoef-30kg-predlen.pdf}
    }
...
\begin{figure}[htb]
    ...
    \captionsetup{aboveskip=0pt}
    \subfigure{
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-decrcoef-30kg-psnr.pdf}
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-decrcoef-30kg-utime.pdf}
    }
...
\begin{figure}[htb]
    ...
    \captionsetup{aboveskip=0pt}
    \subfigure{
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-slowcoef-30kg-psnr.pdf}
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-slowcoef-30kg-utime.pdf}
    }
...
\begin{figure}[htb]
    ...
    \subfigure[Při počáteční velikosti prediktoru 50\,\%]{
        \centering
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-fastcoef-i50-30kg-psnr.pdf}
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-fastcoef-i50-30kg-utime.pdf}
    }
    \subfigure[Při počáteční velikosti prediktoru 3\,\%]{
        \centering
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-fastcoef-i3-30kg-psnr.pdf}
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-fastcoef-i3-30kg-utime.pdf}
...
\begin{figure}[htb]
    ...
    \captionsetup{aboveskip=0pt}
    \subfigure{
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-interval-30kg-psnr.pdf}
        \includegraphics[width=0.5\textwidth]{fig/plot/tuning-interval-30kg-utime.pdf}
    }
...
\begin{itemize}
    \item podle poslední změny (výchozí nastavení, dále jako \emph{last})
    \item medián posledních 3 změn (\emph{med3})
    \item aritmetický průměr posledních 3 změn (\emph{avg3}),
    \item vážený průměr posledních 7 změn (\emph{avg7w}).
\end{itemize}

Dosažené výsledky po 30 tisících generacích jsou na grafech \ref{plotBwalg-30kg}. Ukazuje se, že u~jiných variant, než je základní \emph{last}, je kvalita filtrů srovnatelná. Také potřebný čas běhu je podobný, s~výjimkou varianty \emph{avg7w}, u~které je evoluce v~průměru 2,2krát pomalejší než u~\emph{last}.

\begin{figure}[htb]
    \centering
    \captionsetup{aboveskip=0pt}
    \subfigure{
        \includegraphics[width=0.5\textwidth]{fig/plot/bwalg-30kg-psnr.pdf}
        \includegraphics[width=0.5\textwidth]{fig/plot/bwalg-30kg-utime.pdf}
    }
...
U~parametru $c_{-1}$ je pro nulovou hodnotu program skokově pomalejší než u~okolních hodnot. Při uplatnění tohoto pravidla je pravděpodobně jakákoliv změna velikosti prediktoru (a tím i změna predikované fitness filtrů) výhodnější, než pokud k~žádné úpravě nedojde.

%Obecně se dá říci, že tato sada pravidel je citlivější na vyšší hodnoty koeficientů.
Grafy kvality nalezených filtrů a potřebného času pro různé hodnoty uvedených koeficientů jsou uvedeny v~příloze \ref{apxExpAbs}.
...
\subsection{\todo{popis jednoho behu}}

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.95\textwidth]{fig/plot/run-fitpred1.pdf}
...
\begin{figure}[p]
    ...
    \subfigure[Detektor hran]{
        \includegraphics[width=0.28\textwidth]{fig/cameraman_256.png}
        \hskip2cm
...
Navržený algoritmus se souběžným učením a adaptivními prediktory fitness byl porovnán s~koevolučním CGP s~prediktory fitness pevné délky ($\mathit{FP_{FIX}}$) a se standardním CGP bez koevoluce ($\mathit{CGP_{STD}}$). Experimentovalo se se dvěma nastaveními souběžného učení:

\begin{itemize}
    \item $\mathit{FP_{REL}}$ s~nastavením uvedeným v~tabulce \ref{tabParamsRel} a úpravou velikosti relativně k~současné,
    \item $\mathit{FP_{ABS}}$ s~nastavením uvedeným v~tabulce \ref{tabParamsAbs} a úpravou velikosti dle celkového počtu případů fitness.
\end{itemize}

Ve všech případech byla použita stejná implementace. Stejně jako v~předchozích experimentech byla porovnávána kvalita nalezených obrazových filtrů a délku běhu evoluce. Experimenty byly provedeny na úloze návrhu obrazových filtrů pro obrázky poškozené šumy typu sůl a pepř a výstřelovým šumem o~intenzitách 5 až 80\,\% s~krokem po 5\,\% a také filtrů pro detekci hran.

\begin{table}[h]
    \caption{Zvolené hodnoty parametrů souběžného učení pro $\mathit{FP_{REL}}$}
    \renewcommand{\arraystretch}{1.2}
    \small
...
\begin{table}[h]
    \caption{Zvolené hodnoty parametrů souběžného učení pro $\mathit{FP_{ABS}}$}
    \renewcommand{\arraystretch}{1.2}
    \small
...
% \begin{itemize}
%     \item koeficienty změny velikosti $c_I = 2$, $c_0 = 0,9$, $c_{-1} = 0,96$, $c_{+1} = 1,07$, $c_{+2} = 1$
%     \item změna velikosti prediktoru je relativní k současné velikosti,
%     \item práh nepřesnosti predikce $I_\mathit{threshold} = 1,2$,
%     \item práh nulové rychlosti $v_\mathit{zero} = 0,001$,
%     \item práh pomalého a rychlého růstu fitness $v_\mathit{slow} = 0,1$,
%     \item počáteční velikost prediktorů 3\,\% všech případů fitness,
%     \item náhodná inicializace prediktorů,
%     \item úprava velikosti prediktoru jen při změně rodiče v populaci CGP a na základě poslední změny (varianta \emph{last})
% \end{itemize}
