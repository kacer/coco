...
\begin{table}[htb]
    ...
    \subtable[Relativně k současné velikosti prediktoru]{
        \begin{tabular}{|l|l|l|l|}
            \hline
            \# & podmínka                               & nová hodnota \emph{maxUsedGenes}     & popis \\
            \hline
            1. &  $I > I_\mathit{threshold}$            & $\mathit{maxUsedGenes} \cdot c_I$    & nepřesná predikce     \\
            2. &  $\left|v\right| \leq v_\mathit{zero}$ & $\mathit{maxUsedGenes} \cdot c_0$    & fitness se nemění     \\
            3. &  $v < 0$                               & $\mathit{maxUsedGenes} \cdot c_{-1}$ & fitness klesá         \\
            4. &  $v \leq v_\mathit{slow}$              & $\mathit{maxUsedGenes} \cdot c_{+1}$ & fitness pomalu stoupá \\
            5. &  $v > v_\mathit{slow}$                 & $\mathit{maxUsedGenes} \cdot c_{+2}$ & fitness rychle stoupá \\
            \hline
        \end{tabular}
    }
    ...
    \subtable[Podle celkového počtu případů fitness $N$]{
        \begin{tabular}{|l|l|l|l|}
            ...
            \hline
            1. &  $I > I_\mathit{threshold}$            & $\mathit{maxUsedGenes} \cdot c_I$                     & nepřesná predikce     \\
            2. &  $\left|v\right| \leq v_\mathit{zero}$ & $\mathit{maxUsedGenes} + \left(N \cdot c_0\right)$    & fitness se nemění     \\
            3. &  $v < 0$                               & $\mathit{maxUsedGenes} + \left(N \cdot c_{-1}\right)$ & fitness klesá         \\
            4. &  $v \leq v_\mathit{slow}$              & $\mathit{maxUsedGenes} + \left(N \cdot c_{+1}\right)$ & fitness pomalu stoupá \\
            5. &  $v > v_\mathit{slow}$                 & $\mathit{maxUsedGenes} + \left(N \cdot c_{+2}\right)$ & fitness rychle stoupá \\
            \hline
        \end{tabular}
    }
...
Velikost prediktoru se upravuje vždy při změně rodiče populace filtrů, což v CGP znamená, že některý z potomků má vyšší nebo stejnou predikovanou fitness jako rodič této generace. Také lze volitelně nastavit, že změna velikosti musí nastat minimálně jednou za určitý počet generací.
...
%     \mathit{maxUsedGenes} \leftarrow \left\{
%         \begin{array}{ll}
%             \mathit{maxUsedGenes} \cdot k_I    & \mbox{pokud } I > I_\mathit{threshold} \\
%             \mathit{maxUsedGenes} \cdot k_0    & \mbox{pokud } \left|v\right| \leq v_\mathit{zero} \\
%             \mathit{maxUsedGenes} \cdot k_{-1} & \mbox{pokud } v < -v_\mathit{zero} \\
%             \mathit{maxUsedGenes} \cdot k_{+1} & \mbox{pokud } v_\mathit{zero} < v \leq v_\mathit{fast}   \\
%             \mathit{maxUsedGenes} \cdot k_{+2} & \mbox{pokud } v > v_\mathit{fast}   \\
%         \end{array}
%     \right.
% \end{equation}


\todo{hýbání s offsetem?}
...
\begin{figure}[htb]
    ...
    \subfigure[5\% šum]{
        \centering
            \includegraphics[width=0.9\textwidth]{fig/plot/initial-size/initial-size-5-30kg.pdf}
    }
    \subfigure[15\% šum]{
        \centering
            \includegraphics[width=0.9\textwidth]{fig/plot/initial-size/initial-size-15-30kg.pdf}
    }
    \subfigure[50\% šum]{
        \centering
            \includegraphics[width=0.9\textwidth]{fig/plot/initial-size/initial-size-50-30kg.pdf}
    }
...
%     \renewcommand{\arraystretch}{1.2}
%     \begin{minipage}[t]{.5\textwidth}
%         \small\centering\begin{tabular}{|l|l|}
%             \hline
%             parametr & hodnota \\
%             \hline
...
%             $v_\mathit{slow}$ & 0,1 \\
%             počáteční velikost prediktoru & 3\,\% \\
%             \hline
%         \end{tabular}
